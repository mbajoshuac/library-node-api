...
routeHandler._users.post = (data, callback) => {
    ...
    var borrowed = typeof(data.payload.borrowed) === 'number' && !isNaN(parseInt(data.payload.borrowed)) ? data.payload.borrowed : false;
    if (fullname && email && dob && address && phone && role) {
        ...
        callback(403, { message: "You're sending an incomplete data", data: { fullname, email, dob, address, phone, role, borrowed } })
    }
};
