...
routeHandler._books.post = (data, callback) => {
    ...
    if (userid) {
        ...
        callback(403, { message: 'Please Provide a User(Admin) ID [as a query eg: userid=?] for verification' })
    }
            ...
            if (!err && userData) {
                ...
                if (userData.role === 'admin') {
                    if (data.payload.bookid) {
                        ...
                        callback(404, { message: 'Please enter a valid Book ID (add it to the body object- bookid: ??) that you want to update' });
                    }
