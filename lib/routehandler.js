    ...
    if (userid) {
        ...
        fileUtil.read('users', userid, (err, userData) => {
            if (!err && userData) {
                ...
                callback(403, { message: 'This user account does not exist' });
            }
        })

    } else {
        callback(403, { message: 'Please Provide a User(Admin) ID [as a query] for verification' })
    }
            ...
            if (!err && userData) {
                ...
                if (userData.role === 'admin') {
                    if (data.payload.bookid) {
                        fileUtil.update('books', data.payload.bookid, data.payload, (err) => {
                        ...
                        callback(404, { message: 'Please enter a valid Book ID (as a payload) that you want to update' });
                    }
...
routeHandler._users.post = (data, callback) => {
    ...
    var update = helper.streetAddressRegexp.test(data.payload.address)
    console.log(update);

    //validating user inputs and testing match with regexp.
    var fullname = typeof(data.payload.fullname) === 'string' && data.payload.fullname.trim().length > 0 && helper.fullNameRegexp.test(data.payload.fullname) ? data.payload.fullname : false;
    var email = typeof(data.payload.email) === 'string' && data.payload.email.trim().length > 0 && helper.emailRegexp.test(data.payload.email) ? data.payload.email : false;
    var dob = typeof(data.payload.dob) === 'string' && data.payload.dob.trim() > 0 ? data.payload.dob : false;
    var address = typeof(data.payload.address) === 'string' && data.payload.address.trim().length > 0 && helper.streetAddressRegexp.test(data.payload.address) ? data.payload.address : false;
    var phone = typeof(data.payload.phone) === 'string' && !isNaN(parseInt(data.payload.phone)) && helper.phoneRegexp.test(data.payload.phone) ? data.payload.phone : false;
    var role = typeof(data.payload.role) === 'string' && data.payload.role.trim().length > 0 && data.payload.role === 'user' ? data.payload.role : false;
    if (fullname && email && dob && address && phone && role) {
        ...
        callback(403, { message: "You're sending an incomplete data", data: { fullname, email, dob, address, phone, role } })
    }
};
...
We are sorry.You can\ 't Rent ${data.name} again, It\'s all rented out` })
                    } else {
        ...
        fileUtil.read('users', userid, (err, data) => {
            if (!err && data) {
                ...
                fileUtil.read('books', bookid, (err, data) => {
                    ...
                    book.copies = book.copies + 1
                    fileUtil.update('books', bookid, book, (err, data) => {
                        callback(200, { message: `This Book ${book.name} was successfully Returned by ${user.name}`, data: book })
                    })
                })
            } else {
                callback(403, { message: 'User details not found' })
